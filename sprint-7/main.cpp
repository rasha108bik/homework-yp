...
void TestConstructorOrCapacity() {
    const size_t size = 1000000;
    const int value = 2;
    cout << "Test constructor" << endl;
    SimpleVector<int> vector(size, value);
    assert(vector.GetSize() == size);

    auto list = GenerateVector(size);
    SimpleVector<int> vector_capacity_list(list);
    assert(vector_capacity_list.GetSize() == size);

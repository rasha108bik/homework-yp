...
namespace parse {
    ...
    public:
        ...
        bool CheckLineHasSign(std::string_view line);
        bool parseLine();

        Token ParseLexer();
        Token ParseString(std::string_view str);
        Token ParseNumber(std::string_view str);
        Token ParseIdentifier(std::string_view str);
        Token ParseSign(char ch);

    private:
        std::istream& input_;
        std::string line_;
        bool sign_found_ = false;
        size_t current_pos_ = 0;
        Token token_;

        const std::map<std::string, Token> tokens_{
